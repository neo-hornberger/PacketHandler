...
public class ByteBuffer {
	...
	static {
		...
		addToCache(Schema.class, (buf, schema) -> buf.write(schema, schema));
	...
	/**
	 * Writes the object {@code obj} with the given {@link Schema schema} to the buffer.
	 *
	 ...
	 * @return the current instance
	 *
	 * @see Schema#toBuffer(Object, ByteBuffer)
	 */
	@SuppressWarnings("unchecked")
	public <B extends ByteBuffer, T> ByteBuffer write(final Schema<T, B> schema, final T obj) {
		schema.toBuffer(obj, (B) this);
		
		return this;
	}
	
	/**
	 * Writes the {@link PacketBase} {@code packet} to the buffer.
	 *
	 * @param packet the {@link PacketBase} to write
	 * @param <B>    the {@code ByteBuffer} type the current instance will be casted to
	 *
	 * @return the current instance
	 *
	 * @see PacketBase#toBuffer(ByteBuffer)
	 */
	@SuppressWarnings("unchecked")
	public <B extends ByteBuffer> ByteBuffer writePacket(final PacketBase<B> packet) {
		packet.toBuffer((B) this);
	...
	public <B extends ByteBuffer, T> ByteBuffer writeArray(final T[] objs, final Schema<T, B> schema) {
		Objects.requireNonNull(schema);
		
		return this.<B, T>writeArray(objs, (buf, obj) -> schema.toBuffer(obj, buf));
	}
	 ...
	 * @param writer the {@link Consumer} that defines the writing behavior
	 ...
	 */
	public <T> ByteBuffer writeArray(final T[] objs, final Consumer<T> writer) {
		Objects.requireNonNull(writer);
		
		return writeArray(objs, (_buf_, obj) -> writer.accept(obj));
	}
	...
	@SuppressWarnings("unchecked")
	public <T, B extends ByteBuffer> T read(final Schema<T, B> schema) {
		final T obj = schema.newInstance();
		
		schema.fromBuffer(obj, (B) this);
		
		return obj;
	}
	
	@SuppressWarnings("unchecked")
	public <B extends ByteBuffer, T extends PacketBase<B>> T readPacket(final T packet) {
		packet.fromBuffer((B) this);
	...
	@SuppressWarnings("unchecked")
	public <B extends ByteBuffer, T> T[] readArray(final Schema<T, B> schema) {
		final Class<T> clazz = (Class<T>) schema.newInstance().getClass();
		
		return readArray(arrayFromClassGenerator(clazz), buf -> buf.read(schema));
	}
	
	public <T> T[] readArray(final Class<T> clazz, final Supplier<T> reader) {
	...
	public <T> T[] readArray(final IntFunction<T[]> generator, final Supplier<T> reader) {
		Objects.requireNonNull(reader);
		
		return readArray(generator, (_buf_, _index_) -> reader.get());
	}
