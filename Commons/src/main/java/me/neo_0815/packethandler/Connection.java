...
public abstract class Connection extends PacketSender {
	...
	protected final void sendData(final ByteBuffer buf) {
		...
		try {
			...
			properties().getExceptionHandlingType().handle(this, e);
		}
	}
	...
	/**
	 * Interrupts the {@link ListeningThread} and the {@link PacketQueueThread} and disconnects.
	 *
	 * @see #disconnect()
	 */
	public void stop() {
		ThreadExecutors.stopTask(listeningThread);
		ThreadExecutors.stopTask(packetQueueThread);
	...
	/**
	 * Disconnects from the {@linkplain Server server}.
	 */
