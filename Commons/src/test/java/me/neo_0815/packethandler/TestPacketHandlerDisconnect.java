...
import me.neo_0815.packethandler.Properties.ExceptionHandlingType;
...
public class TestPacketHandlerDisconnect {
	...
	public static void main(final String[] args) throws IOException {
		...
		prop.setExceptionHandlingType(ExceptionHandlingType.STOP);
		...
		final Server server = new Server(8080, prop) {
			...
			protected void onPacketReceived(final UUID client, final PacketBase<?> packet, final long id) {
				...
				if(packet instanceof TestArrayPacket) {
					log("BEFORE");
					
					sendPacket(client, TestPacketType.PACKET, PacketMap.of("object", null));
		...
		// TODO client.sendPacket(TestPacketType.JSON);
		...
		client.sendPacket(TestPacketType.ARRAY);
